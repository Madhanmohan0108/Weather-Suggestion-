...
2. The app calls the **OpenWeather API** to fetch current weather data (temperature, humidity, wind, conditions).
3. That weather data is passed into a prompt sent to the **Gemini API**.
4. Gemini responds as a "30-year veteran weather analyst" with practical clothing and food suggestions tailored to the day's weather.

## Tech Stack

- Python
- [OpenWeather API](https://openweathermap.org/api) — real-time weather data
- [Google Gemini API](https://ai.google.dev/) — LLM-generated suggestions
- `requests` — API calls
...
Built by [Madhanmohan](https://github.com/Madhanmohan0108) as a portfolio project exploring LLM + API integration.
